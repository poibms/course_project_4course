...
final class AddArticleViewController: BaseViewController
{
    ...
    private var category: [String] = ["Nature", "Food", "City", "Friends"]
...
extension AddArticleViewController
{
    @IBAction private func saveButtonPressed(_ sender: UIBarButtonItem)
    {
        ...
        let contentNF = UNMutableNotificationContent()
        contentNF.title = "Someone posted a new photo"
        contentNF.body = "Check out the app to what's new"
        contentNF.sound = .default
        contentNF.userInfo = ["value": "Data with local notification"]
        ...
        let request = UNNotificationRequest(identifier: "reminder", content: contentNF, trigger: trigger)
